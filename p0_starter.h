...
namespace scudb {
    ...
    public:
         ...
         * @return The number of rows in the matrix
         */
        int GetRowCount() const override { return rows_child; }

        /**
         * TODO(P0): Add implementation
         * @return The number of columns in the matrix
         */
        int GetColumnCount() const override { return cols_child; }
        ...
        void FillFrom(const std::vector<T>& source) override {
            ...
            if (source.size() < (rows_child * cols_child)) {
                printf("OUT_OF_RANGE!\n");
            }
            else {
                for (int i = 0; i < rows_child; i++) {
                    for (int j = 0; j < cols_child; j++) {
                        data_[i][j] = source[i * rows_child + j];
                    }
                }
            }
            return;
    ...
    public:
        ...
        static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T>* matrixA, const RowMatrix<T>* matrixB) {
            ...
            if (matrixA->GetRowCount() == matrixB->GetRowCount() && matrixA->GetColumnCount() == matrixB->GetColumnCount()) {
                std::unique_ptr<RowMatrix<T>> result(new RowMatrix<T>(matrixA->GetRowCount(), matrixA->GetColumnCount()));
                for (int i = 0; i < matrixA->GetRowCount(); i++) {
                    for (int j = 0; j < matrixA->GetColumnCount(); j++) {
                        result.get()->SetElement(i, j, matrixA->GetElement(i, j) + matrixB->GetElement(i, j));
                    }
                }
                return result;
            }
            ...
            if (matrixA->GetColumnCount() == matrixB->GetRowCount()) {
                ...
                for (int i = 0; i < matrixA->GetRowCount(); i++) {
                    for (int j = 0; j <matrixB->GetColumnCount(); j++) {
                        ...
                        result.get()->SetElement(i, j, temp);
                    }
                }
                return result;
            }
        ...
        static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T>* matrixA, const RowMatrix<T>* matrixB,
            ...
            if (matrixA->GetColumnCount() == matrixB->GetRowCount() && matrixA->GetRowCount() == matrixC->GetRowCount() && matrixB->GetColumnCount() == matrixC->GetColumnCount()) {
                std::unique_ptr<RowMatrix<T>> result(new RowMatrix<T>(matrixC->GetRowCount(),matrixC->GetColumnCount()));
                for (int i = 0; i < matrixA->GetRowCount(); i++) {
                    for (int j = 0; j < matrixB->GetColumnCount(); j++) {
                        T temp = T();
                        for (int k = 0; k < matrixA->GetColumnCount(); k++) {
                        ...
                        result.get()->SetElement(i, j, temp + matrixC->GetElement(i, j));
                    }
                }
                return result;
            }
